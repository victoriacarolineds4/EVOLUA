...
12. **Negação do classificador de automação (Browser pane) = parar e reportar, nunca reinterpretar como ruído e seguir por outra via.** Já aconteceu de uma ação (ex.: injetar cookie de sessão via `document.cookie` em domínio de produção) ser barrada com "Permission for this action was denied by the Claude Code auto mode classifier", e de uma ação subsequente não relacionada (`navigate` simples) falhar do mesmo jeito e depois funcionar ao repetir. Não usar esse segundo evento como evidência de que a primeira negação foi "flutuação" e trocar de tática sozinho (ex.: logar via formulário digitando senha, quebrando a convenção do item 10) sem consultar antes — mesmo que a ação alternativa pareça inofensiva isoladamente e mesmo que a conta seja descartável. Regra: sempre reportar a mensagem literal da negação e perguntar como proceder, nunca reclassificar por conta própria.
